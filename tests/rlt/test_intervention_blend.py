...
from lerobot.scripts.recording_loop import _blend_robot_actions
...
def test_blend_robot_actions_interpolates_scalar_joints():
    ...
    blended = _blend_robot_actions(list(start), start, target, 0.25)
...
def test_blend_robot_actions_clips_alpha_and_preserves_arrays():
    ...
    low = _blend_robot_actions(["arm.pos"], start, target, -1.0)
    high = _blend_robot_actions(["arm.pos"], start, target, 2.0)
...
def test_blend_robot_actions_surfaces_shape_mismatch():
    ...
    with pytest.raises(ValueError):
        _blend_robot_actions(["arm.pos"], start, target, 0.5)
