...
RECORD_SENT_ACTION_AS_DATASET_ACTION=${RECORD_SENT_ACTION_AS_DATASET_ACTION:-0}
...
export LEROBOT_RECORD_SENT_ACTION_AS_DATASET_ACTION="$RECORD_SENT_ACTION_AS_DATASET_ACTION"
...
status=${PIPESTATUS[0]}
set -e
printf "\nRUN_ID=%s\nROOT=%s\nLOG=%s\nDEBUG_JSONL=%s\nBRIDGE_ANCHOR=%s\nMAX_RELATIVE_TARGET=%s\nSYNC_POLICY_LAST_ACTION_FROM_SENT=%s\nRECORD_SENT_ACTION_AS_DATASET_ACTION=%s\n" "$RUN_ID" "$ROOT" "$LOG" "$ROOT/action_state_debug.jsonl" "$BRIDGE_ANCHOR" "${MAX_RELATIVE_TARGET:-none}" "$SYNC_POLICY_LAST_ACTION_FROM_SENT" "$RECORD_SENT_ACTION_AS_DATASET_ACTION"
exit "$status"
